...
class CFG:
    ...
    #TRAIN = "train"
    #VAL = "val"
    train_path = '/ddsmDataset/classification_BM_DDSM_crop/train'
    validate_path = '/ddsmDataset/classification_BM_DDSM_crop/val'
    ...
    def fit(self, model, trainloader, validloader, epochs):
        ...
        for i in range(epochs):
            ...
            if avg_valid_acc > valid_max_acc:
                ...
                torch.save(
                    model.state_dict(), '/ddsmDataset/classification_BM_DDSM_crop/ColabDDSMModel_bestAcc_BM.pt')
...
model.load_state_dict(torch.load(
    '/ddsmDataset/classification_BM_DDSM_crop/ColabDDSMModel_bestAcc_BM.pt'))
model.eval()

avg_test_loss, avg_test_acc = trainer.valid_batch_loop(model, validationloader)
...
for i, testEle in enumerate(testset):
    ...
    resultText = open("./result/" + testset.imgs[i][0].split("/")[-1].replace(".jpg",".txt"),'w')
    resultConfid = ps.cpu().data.numpy().squeeze()
    resultText.write(str(resultConfid[0]) + " " + str(resultConfid[1]))
    resultText.close()
