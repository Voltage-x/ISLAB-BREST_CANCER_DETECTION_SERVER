...
class CFG:
  ...
  epochs = 300                              # No. of epochs for training the model
  lr = 0.001                              # Learning rate
  batch_size = 16                         # Batch Size for Dataset

  model_name = 'tf_efficientnetv2_l_in21k'    # Model name (we are going to import model from timm)
  img_size = 480                          # Resize all the images to be 224 by 224
...
train_transform = T.Compose([
                             ...
                             T.RandomAffine(degrees=(-90, +90), translate=(0.1, 0.3), scale=(0.5, 1.5)),
...
model.classifier = nn.Sequential(nn.Linear(in_features=1280, out_features=2,bias=True))
    ...
    def fit(self,model,trainloader,validloader,epochs):
        ...
        for i in range(epochs):
            ...
            if avg_valid_acc > valid_max_acc:
                ...
                torch.save(model.state_dict(),'/ddsmDataset/classification/ColabDDSMModel_bestAcc.pt')
...
model.load_state_dict(torch.load('/ddsmDataset/classification/ColabDDSMModel_bestAcc.pt'))
model.eval()
